...
export default class {
  ...
  handleShowTickets(e, bills, index) {
    ...
    if (this.counter % 2 === 0) {
      ...
      $(`#arrow-icon${this.index}`).attr('data-open', 'true')
      $(`#status-bills-container${this.index}`)
        .html(cards(filteredBills(bills, getStatus(this.index))))
      this.counter++
    } else {
      $(`#arrow-icon${this.index}`).css({ transform: 'rotate(90deg)' })
      $(`#arrow-icon${this.index}`).attr('data-open', 'false')
      $(`#status-bills-container${this.index}`)
        .html("")
      this.counter++
    }

    // unbind previous click event to avoid conflicts when new bill is selected
    $('.bill-card').off('click');
